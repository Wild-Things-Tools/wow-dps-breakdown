...
**The git history is deliberately NOT rewritten** -- the owner's decision, 12.09.2026,
asked as a choice between leaving it, rewriting with `git-filter-repo`, and measuring
first. Leaving it is the answer, and the reasons are worth keeping because the
flattering reading is that a rewrite "removes" them: it does not. A force-pushed
rewrite of a public repository leaves the old objects in every existing clone, can be
served on from GitHub's own caches and any fork, and breaks every commit sha anybody
has referenced -- while the thing being removed is a guild id, which resolves to a
name and a realm and is not a personal identifier. So the trade is a certain,
irreversible cost against a partial removal. What is done instead is the part that
actually holds: the current file carries none, the projection converts any that
return, and this entry says where they are.
...
### A re-read that found fewer kills overwrote the row that found more (#171)

#159 closed the **zero** case -- an encounter the ceiling stopped before any fight
contributes no row at all. It left the rest of `by_id = {**previous, **fresh}`
exactly as it was, and that is a flat replacement: a re-read returning *fewer*
kills than the previous row still wins, because it is the later dict. #159's own
entry says so in words -- *"Keep the bigger row is a different question with its
own machinery downstream"* -- and names the place it would be answered. This is
that place.

**The owner's decision (13.09.2026) is union, and only when the settings agree.**
The choice put to him was between replacing, unioning unconditionally, and
unioning only over rows chosen the same way; the third is what `merge_entry`
implements.

**The union is at the ROW, and that is forced rather than chosen.** The obvious
reading of "union" is the fight lists -- keep every distinct `(reportCode,
fightId)` across both reads. It cannot be done faithfully from a payload, and the
reason is a measurement: `FightObservation.to_json` emits neither `friendly_ids`
nor the actor names and game ids, so a `FightObservation` rebuilt from a stored
row carries an **empty friendly set**. `is_encounter_aura` then reads
`aura.actor_id in self.friendly_ids` as False and `source_id in friendly_ids` as
False, so **every player aura passes as an encounter mechanic** -- which is the
Avenging Wrath failure this file already records twice, reintroduced by a merge
rather than by a filter. A fight-level union therefore needs a lossy reverse
parser that would silently break the aura list, and the honest merge is the one
that keeps whole rows.

Three decisions in `merge_entry`, each of which reversed produces a plausible row
rather than an error:

- **Unknown is not equal.** `_same_selection` compares `order` and `eventBudget`
  and returns False when **either side states neither**. A row written before
  those fields existed cannot say how its kills were chosen, and treating silence
  as agreement would union a page-1-by-damage sample into a first-kills-by-date
  one under the second one's label. The cost is that such a row is replaced, which
  is what happened before this existed.
- **The fresh read wins whenever it is a superset**, including when the two are
  equal -- so an ordinary re-read still refreshes `searchExhausted`,
  `searchBudget` and every field beside the kills, and the bytes move exactly as
  they did. Only a **strict** shrink keeps the older row, and it takes the newer
  walk's two findings with it (`searchExhausted` once set stays set; `searchBudget`
  is the larger of the two), because those are claims about the *search* rather
  than about the kills.
- **The disjoint case is named, never hidden.** Two reads that share no kill at
  all cannot both be kept in one row, so the larger is taken and the count that
  fell out is added to `mergeDroppedKills` on the row. It is the `staleRows` rule
  and the `unplaced` rule again: report what a merge could not hold rather than
  keeping or dropping it quietly. Nothing produces this state today -- both reads
  come from the same ranking of the same boss -- which is exactly why it is
  counted rather than trusted not to happen.

**Nothing published moves until a probe run writes.** The change is in what a
*payload* row becomes; `fights.json` is rebuilt from the payload on the next
`--publish`, and a run in which every encounter is a superset of its predecessor
produces the bytes it produced before.

**Seven tests and a canary each.** The one worth keeping is the end-to-end one:
the fold is a pure function over two dicts and passes trivially, so the claim that
the *command* stops overwriting needs the command --
`test_a_shorter_re_read_keeps_the_bigger_row_through_the_command` drives
`cmd_fight_probe` with a stub whose ceiling falls after the first fight, which is
the shape that produced the defect. That is the same lesson #159's fourth canary
taught one entry up, applied before it could be taught again.

And one existing test moved rather than being deleted.
`test_a_partial_read_is_still_written_and_that_boundary_is_deliberate` used to
hand its command a previous row of three kills and assert the partial read of one
replaced it -- which is precisely the behaviour this decision reverses. Its own
claim (a partial read is *written*, not withheld) is unchanged and still has to
hold, so its fixture has no previous row now, and the two rules cannot collide
inside one test.

### A caveat lives inside the carried block, so a wording fix reaches one difficulty

Measured on 2026-09-12, within hours of #174 merging, and it is a consequence of
`_keep_measurements` that nobody had stated.

The `public`-order caveat fix landed on `main` at 14:45. The hourly `fight-probe`
at 19:29 regenerated `fights.json`, and the published document then read:

```
                     before (13 blocks with fights)   after
"page 1 of the rankings"          13                    6
"earliest kills by kill date"      0                    7
```

Not 13 of 13 -- and the split is **exactly** by difficulty:

```
every Mythic (5) block    the corrected sentence
every Heroic (4) block    the old one
```

**That is correct, and it is the carry-forward rule doing its job.** The run was
`--difficulty 5`, so `_keep_measurements` brought every Heroic block back from the
published document **verbatim** -- and a block's `caveats` are part of the block.
So a fix to how a caveat is *worded* reaches only the difficulties a run actually
reads; every other block keeps the sentence it was published with.

The general form, and it applies to any future presentation fix in this file:
**regenerating the document is not the same as regenerating a block.** A code change
to `_caveats`, `_target_band`, `_timeline_block` or anything else inside the per-block
build only reaches a `(encounter, difficulty)` pair on a run that reads that pair. The
document-level fields move on every publish; the blocks do not.

Two things follow. A `--publish --resume` dispatch at the *other* difficulty is the
cheap way to finish such a fix, since the stored payload already holds those entries
and the rebuild costs no query on a complete encounter. And a reader comparing two
blocks of one boss can see two generations of the same sentence side by side, which
looks like an inconsistency and is a date.

Not dispatched here: a Heroic pass shares the hourly Warcraft Logs budget with the
`progress-sweep` that was running at the time, and six carried caveat lines are not
worth contending for it. The blocks correct themselves on the next Heroic read.
