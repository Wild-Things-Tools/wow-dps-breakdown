...
**This repository is the calculator; the product surface is [nextpull](https://nextpull.gg).**
Owner's decision of 2026-08-29. Everything a reader sees is drawn there, from the
documents committed under `web/public/data/`, which nextpull fetches through its own
backend. **Committing a dataset is the whole publishing step** — there is no deploy to
wait for, and `deploy.yml` no longer runs on its own (see the workflow list below).

`web/` stays, and deleting it would be a mistake: its TypeScript types and vitest suite
are what pin the shape of every published document, and `npx vite preview` is the cheapest
way to look at a dataset before shipping it. It is the pipeline's own inspection tool, not
a second product — it needs to compile and agree with the schema, and nothing more.

So: **new view work goes to nextpull**, not here. A chart built in `web/` is a chart on a
surface nobody is served.

Being public is deliberate and load-bearing — it is what makes the simulation workflows
free to run (see *Actions minutes* below). Nothing may move into this repository that
could not be public.
...
.github/workflows/
  ...
  deploy.yml         stood down; dispatch-only (was: push the site into wt-gate)
...
- **Stood down** (`deploy.yml`): used to rebuild the site on every push to `main` and
  push it into the `wt-gate` repository. **It no longer runs on its own** — the trigger
  is `workflow_dispatch` only, since 2026-08-30. The product surface is nextpull, and a
  deploy into a project nobody serves is git-history growth in another repository.
  Everything below about tokens, variables and Cloudflare is kept as the record of how
  it worked, so re-opening the route is a dispatch rather than an archaeology exercise.

  What it *was*: `wt-gate` is a Cloudflare Pages project behind a Discord login, so
  Cloudflare rebuilt it on that push and the site appeared at `<wt-gate domain>/wow-dps/`
  — visible to guild members with an allowed role, and to nobody else.

  This is also why the repository could stay private: Cloudflare Pages serves from private
  repositories, unlike GitHub Pages, which needs Enterprise for that. It is public today,
  which is what makes the simulation workflows free to run.
