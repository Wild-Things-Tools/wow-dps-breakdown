...
def test_a_hand_excluded_mythicplus_trinket_drops_out_of_the_pool():
    """The lever for a rotation nobody can derive: name last season's trinket ids
    and they stop anchoring a baseline. Only Mythic+ items, so a raid drop with the
    same id can never be culled by accident."""
    from wowdps.equipment import TRINKET, GearItem, ItemLevel, SlotPool

    def trinket(item_id, source):
        return GearItem(
            item_id=item_id,
            name=f"T{item_id}",
            slug=f"t{item_id}",
            ...
            secondary_stat=None,
            source=source,
            base_ilevel=108 if source == "mythicplus" else 219,
            base_quality=3,
        )
    ...
    pool = SlotPool(
        tier="MID2",
        slot=TRINKET,
        items=(trinket(1, "mythicplus"), trinket(2, "mythicplus"), trinket(1, "raid")),
        item_levels=(ItemLevel("heroic", "H", 334, ""),),
        baseline_source="mythicplus",
        candidate_source="raid",
        excluded_ids=frozenset({1}),
    )
    # Mythic+ id 1 excluded; id 2 kept; the raid id 1 untouched (different source).
    assert pool.in_rotation(trinket(1, "mythicplus")) is False
    assert pool.in_rotation(trinket(2, "mythicplus")) is True
    assert pool.in_rotation(trinket(1, "raid")) is True


def test_a_derived_rotation_answer_overrides_the_hand_exclusion():
    """Blizzard's own loot table wins: an exclusion the derive contradicts is a
    finding for a human, not the last word."""
    ...
    assert pool.in_rotation(item) is True
