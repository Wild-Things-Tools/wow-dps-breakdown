...
from wowdps import equipment, simc_runner
...
# --------------------------------------------------------------------------------
# ... and, for a profile that states none, read back out of simc
# --------------------------------------------------------------------------------


def test_derive_asks_simc_to_regenerate_the_block_the_profile_lacks(tmp_path, monkeypatch):
    """A materialised profile carries no gear summary, so the question goes to simc.

    The point is *whose* arithmetic answers it: simc regenerates the block with the
    code that wrote every shipped profile's, rather than this repository keeping a
    class/spec table `primary_stat` deliberately refuses to keep.
    """
    path = write_profile(tmp_path, "paladin=X\nspec=retribution\n")
    asked: list[Path] = []

    def regenerate(simc, profile, timeout=120):
        asked.append(profile)
        return "paladin=X\n# Gear Summary\n# gear_strength=1361\n# gear_stamina=900\n"

    monkeypatch.setattr(simc_runner, "regenerate_profile", regenerate)
    assert equipment.derive_primary_stat(Path("simc"), path) == "strength"
    assert asked == [path]


def test_derive_refuses_when_simcs_own_export_states_no_stat_either(tmp_path, monkeypatch):
    """Two silences, one answer: still no guess.

    An export with no summary line means simc could not say either -- which is a
    refusal, not a default. Returning the commonest stat here would put a plausible
    wrong primary stat on exactly the builds nobody checks.
    """
    path = write_profile(tmp_path, "paladin=X\n")
    monkeypatch.setattr(simc_runner, "regenerate_profile", lambda *a, **k: "paladin=X\n")
    with pytest.raises(ValueError, match="regenerated by simc"):
        equipment.derive_primary_stat(Path("simc"), path)


def test_a_refused_export_reaches_the_caller_as_the_simc_error(tmp_path, monkeypatch):
    """`SimcError`, not `ValueError`: "simc would not run this" and "simc ran it and
    said nothing" are different findings, and only the second is about the profile's
    stats. Measured: MID2_Paladin_Retribution exits 81 on its own talent hash."""

    def refuse(*args, **kwargs):
        raise simc_runner.SimcError("simc exited 81 regenerating MID2_Paladin_Retribution.simc")

    path = write_profile(tmp_path, "paladin=X\n")
    monkeypatch.setattr(simc_runner, "regenerate_profile", refuse)
    with pytest.raises(simc_runner.SimcError):
        equipment.derive_primary_stat(Path("simc"), path)
