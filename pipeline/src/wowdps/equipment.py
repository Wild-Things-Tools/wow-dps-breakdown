...
def primary_stat_in(text: str, label: str) -> str:
    """The primary stat a ``# Gear Summary`` block describes.

    Split out of :func:`primary_stat` so the same reading serves a profile on disk
    and one simc has just regenerated -- the two differ in where the block comes
    from and not in what it means, and a second copy of this parse would be a
    second place for the tie rule below to drift.

    Specs carry a little of another primary (Elemental Shaman has 94 strength from
    an off-hand, Vengeance 442 intellect), so the largest allocation wins rather
    than the only one.
    """
    found = [(float(value), stat) for stat, value in _GEAR_STAT.findall(text)]
    if not found:
        raise ValueError(
            f"{label} has no '# gear_<stat>=' summary line, so its primary "
...
def primary_stat(profile_path: Path) -> str:
    ...
    a table would need editing every time a spec changes hands.

    A profile this project *materialises* carries no such block -- simc's generator
    writes it only into the profiles it ships -- so this raises for those, and
    :func:`derive_primary_stat` asks simc to regenerate it. The profile stays the
    authority either way; only the profile's silence sends the question on.
    """
    return primary_stat_in(
        profile_path.read_text(encoding="utf-8", errors="replace"), profile_path.name
    )


def derive_primary_stat(simc: Path, profile_path: Path, timeout: int = 120) -> str:
    """The primary stat of a profile whose own gear summary is missing.

    simc regenerates the block from the equipped items with the same code that
    wrote every shipped profile's, so this is the same quantity by the same
    authority rather than a second opinion -- which is what makes it safe to
    publish beside the profiles that state it themselves.

    Imported lazily because this module is otherwise pure: it reads text and
    tables and runs nothing. Only this one function needs a subprocess, and
    ``simc_runner`` is where invoking simc lives.

    Measured on simc ``b0ea612``, 2026-08-30, over the eleven materialised MID2
    profiles that carry no summary: ten derive at 0.11 s each -- Balance Druid
    ``intellect``, Feral ``agility``, Retribution ``strength`` -- and one
    (``MID2_Paladin_Retribution``, whose talent hash simc's own parser refuses)
    exits 81 and derives nothing, which is a refusal rather than a value.
    """
    from . import simc_runner

    return primary_stat_in(
        simc_runner.regenerate_profile(simc, profile_path, timeout=timeout),
        f"{profile_path.name} (regenerated by simc)",
    )
